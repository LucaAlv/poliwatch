...
- `fact_metrics`, `facts`, `fact_sources`: the weekly "Fakt der Woche" observations, computed at build time by `scripts/facts.py` from the rest of the store (percentiles, baselines and their receipts). Not sourced from DIP, bundestag.de or abgeordnetenwatch.de; `data/exports/datenstand.json` marks every column of these three tables `derived`.

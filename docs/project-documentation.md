...
The renderer owns the detailed dossier page HTML, shared site header styles, party colors, vote labels, source links, speaker/profile links, summary presentation, and a footer nav whose Daten link only appears when the build actually has data to show (`database_page_href`, predicted ahead of the export by `dossier_database_page_href()` in `build_dip_pulse_site.py` from the same conditions the SQLite export uses).

The dossier's Aufmerksamkeitsrang sidebar lives here too. On desktop it is sticky and its rows scroll inside the panel; at 1120px and below it renders collapsed to `ATTENTION_PREVIEW_ROWS` rows (`ATTENTION_PREVIEW_ROWS_PHONE` at 720px and below) behind an expand button, protocols without agenda items or extracted speeches get a plain notice instead of a ranking, and print output shows every row without controls. Ranking row labels strip recognized procedural boilerplate from the start of the agenda heading and keep the full normalized heading in the link's `title` tooltip. Both row counts are module constants near the top of the file. An `IntersectionObserver` in `attention_runtime_script()` also tracks which ranking row is "current" as the reader scrolls, setting `aria-current="true"` on the matching `.attention-row` and scrolling it into view inside the list; it switches off wherever the static-layout media query applies (`ATTENTION_STATIC_LAYOUT_QUERY`, shared between the CSS and the script) and in print, since there is no rail to mark "you are here" on.

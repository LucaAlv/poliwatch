...
def render_activities(item: dict[str, Any]) -> str:
    ...
    if not activities:
        return '<span class="status warn">No API activity match</span>'
    rows = []
    for activity in activities:
        ...
        person_text = f"Person {person}" if person else "No person ID"
        page_text = f"page {page}" if page else "no page"
        rows.append(
            "<li>"
...
def render_json_details(title: str, payload: Any, class_name: str = "api-json") -> str:
    ...
    if empty:
        return f'<details class="{class_name}"><summary>{esc(title)}{esc(count)}</summary><pre>No API records.</pre></details>'
...
def render_protocol_api_overview(report: dict[str, Any]) -> str:
    ...
    people_html = '<span class="muted">No person records fetched.</span>'
    if people_rows:
    ...
    return f"""
      <section class="api-overview">
        <div>
          <h2>API Records</h2>
          <p>DIP API payloads preserved in the generated JSON. Compact tables below keep the page scannable; expandable JSON blocks expose the original fields.</p>
        </div>
        <div class="api-record-grid">
          {render_json_details("Protocol record", records.get("protocol"))}
          {render_json_details("All Vorgangsposition records", records.get("vorgangspositionen"))}
          {render_json_details("All Aktivität records", records.get("aktivitaeten"))}
          {render_json_details("Person records", records.get("persons"))}
          {render_json_details("Roll-call vote candidates", records.get("roll_call_vote_candidates"))}
          {render_json_details("Matched roll-call votes", records.get("matched_roll_call_votes"))}
        </div>
        <section class="people-section">
          <h3>People From Activities</h3>
          {people_html}
        </section>
      </section>
    """
        ...
        top_sections.append(
            ...
            <article class="top-card" id="top-{item['index']}">
              ...
              <section class="raw-top-api">
                <h3>Raw Matched API Records</h3>
                <div class="api-record-grid">
                  {render_json_details("Matched positions", item.get('api', {}).get('raw', {}).get('positions'))}
                  {render_json_details("Matched activities", item.get('api', {}).get('raw', {}).get('activities'))}
                  {render_json_details("Votes", item.get('votes'))}
                </div>
              </section>
  ...
  <div class="shell">
    <header>
      ...
      <div class="meta-grid">
        ...
        <div class="metric"><span>People Records</span><strong>{esc(summary.get('person_record_count', summary.get('unique_person_ids')))}</strong></div>
      </div>
    </header>

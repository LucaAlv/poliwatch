...
def safe_href(url: Any) -> str | None:
    """Return an allowlisted Bundestag https URL, otherwise omit the link."""
    text = str(url or "").strip()
    if not text:
        return None
    try:
        return publication.validate_external_url(text, "bundestag-dip")
...
def render_session_llm_summary(
    ...
    pdf_url = safe_href(protocol.get("pdf_url"))
    rows = []
